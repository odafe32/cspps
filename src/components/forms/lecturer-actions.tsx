...
export function LecturerActions({
  lecturerId,
  lecturerName,
  email,
  department,
  isActive,
}: LecturerActionsProps) {
  ...
  const [editForm, setEditForm] = useState({
    ...
    lastName: lecturerName.split(" ").slice(1).join(" ") || "",
    email,
  ...
  async function handleEdit() {
    ...
    try {
      ...
      toast.success("Lecturer Updated", { description: `${lecturerName} has been updated` });
      setEditOpen(false);
      router.refresh();
    } catch {
  ...
  async function handlePasswordReset() {
    ...
    try {
      ...
      toast.success("Password Reset", { description: `New password set for ${lecturerName}` });
  ...
  async function handleToggleStatus() {
    ...
    try {
      ...
      toast.success(isActive ? "Lecturer Suspended" : "Lecturer Activated", {
        description: `${lecturerName} has been ${isActive ? "suspended" : "activated"}`,
      });
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.success("Lecturer Deleted", { description: `${lecturerName} has been removed` });
      setDeleteOpen(false);
      router.refresh();
    } catch {
      toast.error("Network Error");
  ...
  return (
    <>
      ...
      <Dialog open={passwordOpen} onOpenChange={setPasswordOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Set a new password for {lecturerName}</DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
      ...
      <Dialog open={suspendOpen} onOpenChange={setSuspendOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{isActive ? "Suspend Lecturer" : "Activate Lecturer"}</DialogTitle>
            <DialogDescription>
              {isActive
                ? `${lecturerName} will not be able to log in until reactivated.`
                : `${lecturerName} will be able to log in again.`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:gap-0">
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete {lecturerName}? This will permanently remove the account. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:gap-0">
